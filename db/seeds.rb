...
def precinct_results_export
   State.all.each do |state|
    ...
    major_results = statecandidate_results.where(candidate_id: top_three).order('precincts.id').group(['precincts.id', 'candidates.id']).sum(:total)
    ...
    CSV.open("#{state.name}-precinct-results.csv", "wb") do |csv|
